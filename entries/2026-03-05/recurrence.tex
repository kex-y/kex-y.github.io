...
\begin{proof}
  ...
  Defining \(\tau_0 = 0\)
  \[\tau_{n + 1} := \inf\{t > \tau_n : |B_t| \in 2^{\mathbb{Z}} \setminus \{|B_{\tau_n}|\}\}\]
  ...
  it follows that the conclusion holds if we can show that \(X_t\) is symmetric (and so, is recurrent). 

  Straightaway, by scaling invariance and the strong Markov property, it suffices to show that 
  \[\mathbb{P}(X_{\tau_1} = 0) = \mathbb{P}(X_{\tau_1} = 2) = \frac{1}{2}.\]
